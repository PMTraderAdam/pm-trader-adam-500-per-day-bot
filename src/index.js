/**
 * Polymarket 5m crypto Up/Down bot – late-window resolution snipe.
 * Markets: BTC, ETH, SOL, XRP (5-minute Up/Down windows).
 * Buys the market favorite near window end (e.g. @0.98–0.99) and holds to resolution.
 * Uses live prices for entry/exit simulation. Logs to console and logs.txt.
...
/** Normalize and validate PM_PRIVATE_KEY. Prepends 0x if missing. Exits if invalid. */
function checkEnvConfig() {
    let pk = process.env.PM_PRIVATE_KEY?.trim();
    if (!pk) {
        console.error("PM_PRIVATE_KEY is required. Set it in your .env file and try again.");
        process.exit(1);
    }
    if (!pk.startsWith("0x"))
        pk = "0x" + pk;
    ...
    if (!validHex64.test(pk)) {
        console.error("PM_PRIVATE_KEY is invalid. It must be 64 hex characters (with or without 0x prefix). Fix it in your .env file and try again.");
        process.exit(1);
    }
    process.env.PM_PRIVATE_KEY = pk;
}
...
async function getWalletBalanceUsdc() {
    const pk = process.env.PM_PRIVATE_KEY?.trim();
...
// Late-window entry: buy the favorite when the outcome is nearly decided.
const ENTRY_TIME_MIN = 250;
const ENTRY_TIME_MAX = 290;
const ENTRY_PRICE_MIN = 0.97;
const ENTRY_PRICE_MAX = 0.99;
const RESOLVE_SEC = 298; // redeem held shares at window end
const RESOLVE_WIN_BID_MIN = 0.9; // side still favored → simulate $1 payout
const POLL_MS = 2000;
const POLL_MS_LATE = 1000; // faster polling once the entry window opens
const LATE_POLL_FROM_SEC = 240;
const HEARTBEAT_EVERY_POLLS = 15;
const MIN_HOLD_SEC = 2;
const SLIPPAGE_BPS = 50; // 0.5% slippage on loss exits only
function sleep(ms) {
...
function get5mMarketStart(nowSec) {
    return Math.floor(nowSec / 300) * 300;
}
function get5mSlug(asset, nowSec) {
...
async function getMarket(slug) {
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
            ...
            if (!j || typeof j.clobTokenIds !== "string")
                return null;
            let ids;
            try {
...
async function getPrices(tokenIds) {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
            const r = await fetchWithTimeout("https://clob.polymarket.com/prices", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(unique.flatMap((token_id) => [
                    { token_id, side: "BUY" },
                    { token_id, side: "SELL" },
                ])),
            });
...
function printFinalResult(balance, totalPnL, tradeCount, startTime, startBalance) {
...
async function main() {
    ...
    if (walletBalance == null) {
        log(`  [INFO] Using default start balance $${DEFAULT_START_BALANCE}. Set PM_PRIVATE_KEY in .env to use wallet balance.`);
    }
    else {
    ...
    while (true) {
        ...
        const tokensByAsset = {};
        const marketResults = await Promise.all(MARKETS.map(async (asset) => {
        ...
        if (pollCount % HEARTBEAT_EVERY_POLLS === 0) {
            const parts = MARKETS.map((asset) => {
                ...
                if (pos)
                    return `${MARKET_LABEL[asset]} hold ${pos.side === "up" ? "Up" : "Down"}@${pos.entryPrice.toFixed(2)}`;
                return `${MARKET_LABEL[asset]} Up=${upBuy.toFixed(2)} Down=${downBuy.toFixed(2)}`;
            });
            const anyEntry = MARKETS.some((asset) => {
                const tokens = tokensByAsset[asset];
                if (!tokens || positions[asset])
                    return false;
                const upBuy = clampPrice(Number(prices[tokens.upTokenId]?.BUY ?? 0.5));
                const downBuy = clampPrice(Number(prices[tokens.downTokenId]?.BUY ?? 0.5));
                return inEntryTime && ((upBuy >= ENTRY_PRICE_MIN && upBuy <= ENTRY_PRICE_MAX) ||
        ...
        if (sec >= LATE_POLL_FROM_SEC)
            await sleep(POLL_MS_LATE);
        else
            await sleep(POLL_MS);
    }
}
